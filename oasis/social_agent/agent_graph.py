...
class AgentGraph:
    ...
    def add_agent(self, agent: SocialAgent):
        if self.backend == "igraph":
            # 【!! 修正 !!】
            # 不再使用 agent_id 作为 igraph 的内部索引。
            # 而是将 agent_id 存储为 'name' 属性。
            # igraph 会自动分配一个内部索引 (0, 1, 2...)
            self.graph.add_vertex(name=str(agent.social_agent_id))
        else:
            self.graph.create_agent(agent.social_agent_id)
        
        # agent_mappings 字典按我们的 agent_id 存储 (e.g., 829)，这是正确的。
    ...
    def remove_agent(self, agent: SocialAgent):
        if self.backend == "igraph":
            # 【!! 修正 !!】
            # 必须通过 'name' 属性来查找并删除节点。
            self.graph.delete_vertices(
                self.graph.vs.select(name_eq=str(agent.social_agent_id))
            )
        else:
    ...
    def get_agents(
        ...
        if self.backend == "igraph":
            # 【!! 修正 !!】
            # 这是导致崩溃的地方。
            # 我们不使用 node.index (它是 0, 1, 2...)
            # 我们使用 node["name"] (它是 "829", "222"...)
            # 并将其转换回 int 作为字典的键。
